...
// 32-bit
uint32_t rz_atomic32_load(RZAtomicU32* atom, RazixMemoryOrder order)
{
    u32 value = *atom;
...
void rz_atomic32_store(RZAtomicU32* atom, uint32_t value, RazixMemoryOrder order)
{
...
uint32_t rz_atomic32_add(RZAtomicU32* atom, uint32_t value, RazixMemoryOrder order)
{
...
uint32_t rz_atomic32_sub(RZAtomicU32* atom, uint32_t value, RazixMemoryOrder order)
{
    return rz_atomic32_add(atom, -(int32_t) value, order);
}

uint32_t rz_atomic32_increment(RZAtomicU32* atom, RazixMemoryOrder order)
{
    return InterlockedIncrement((volatile long*) atom);
}

uint32_t rz_atomic32_decrement(RZAtomicU32* atom, RazixMemoryOrder order)
{
    return InterlockedDecrement((volatile long*) atom);
}

bool rz_atomic32_cas(RZAtomicU32* atom, uint32_t expected, uint32_t desired, RazixMemoryOrder order)
{
...
uint64_t rz_atomic64_load(RZAtomicU64* atom, RazixMemoryOrder order)
{
    uint64_t value = *atom;
    if (order == RZ_MEMORY_ORDER_ACQUIRE ||
        order == RZ_MEMORY_ORDER_ACQ_REL ||
...
void rz_atomic64_store(RZAtomicU64* atom, uint64_t value, RazixMemoryOrder order)
{
    if (order == RZ_MEMORY_ORDER_RELEASE ||
        order == RZ_MEMORY_ORDER_ACQ_REL ||
        order == RZ_MEMORY_ORDER_SEQ_CST) {
...
uint64_t rz_atomic64_add(RZAtomicU64* atom, uint64_t value, RazixMemoryOrder order)
{
...
uint64_t rz_atomic64_sub(RZAtomicU64* atom, uint64_t value, RazixMemoryOrder order)
{
    return rz_atomic64_add(atom, -(int64_t) value, order);
}

uint64_t rz_atomic64_increment(RZAtomicU64* atom, RazixMemoryOrder order)
{
    long long old = InterlockedIncrement64((volatile long long*) atom);
    return (uint64_t) old;
}

uint64_t rz_atomic64_decrement(RZAtomicU64* atom, RazixMemoryOrder order)
{
...
void* rz_atomic_ptr_load(RZAtomicUPtr* atom, RazixMemoryOrder order)
{
    return (void*) rz_atomic64_load((RZAtomicU64*) atom, order);
}

void rz_atomic_ptr_store(RZAtomicUPtr* atom, void* value, RazixMemoryOrder order)
{
    rz_atomic64_store((RZAtomicU64*) atom, (uint64_t) value, order);
}

void* rz_atomic_ptr_exchange(RZAtomicUPtr* atom, void* desired, RazixMemoryOrder order)
{
    return (void*) rz_atomic64_exchange((RZAtomicU64*) atom, (uint64_t) desired, order);
}

bool rz_atomic_ptr_cas(RZAtomicUPtr* atom, void* expected, void* desired, RazixMemoryOrder order)
{
...
static inline int rz_map_memorder(RazixMemoryOrder order)
{
    switch (order) {
