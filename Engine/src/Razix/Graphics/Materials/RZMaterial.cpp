...
namespace Razix {
    namespace Graphics {
        ...
        RZMaterial::RZMaterial(RZShaderHandle shader)
        {
            ...
            m_MaterialPropertiesUBO = RZResourceManager::Get().createUniformBuffer({"Material Properties UBO", sizeof(MaterialProperties), &m_MaterialData.m_MaterialProperties, BufferUsage::Staging});
        }
        ...
        void RZMaterial::InitDefaultTexture()
        {
            ...
            s_DefaultTexture = RZResourceManager::Get().createTexture({.name = "Default Texture", .width = 1, .height = 1, .data = pinkTextureDataRaw, .size = sizeof(u8) * 4, .format = TextureFormat::RGBA8});
