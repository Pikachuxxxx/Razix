...
namespace Razix {
    namespace Graphics {
        ...
        public:
            ...
            RAZIX_FORCE_INLINE static void Init() { RAZIX_PROFILE_GPU_SCOPE("Init Rendering"); s_APIInstance->InitAPIImpl(); }
            RAZIX_FORCE_INLINE static void Begin() { RAZIX_PROFILE_GPU_SCOPE("Begin Rendering"); s_APIInstance->BeginAPIImpl(); }
            RAZIX_FORCE_INLINE static void Present(RZCommandBuffer* cmdBuffer) { RAZIX_PROFILE_GPU_SCOPE("Submit and Present"); /*RAZIX_PROFILE_GPU_FLIP(getSwapchain()->GetAPIHandle());*/  s_APIInstance->PresentAPIImple(cmdBuffer); }
            RAZIX_FORCE_INLINE static void BindDescriptorSets(RZPipeline* pipeline, RZCommandBuffer* cmdBuffer, std::vector<RZDescriptorSet*>& descriptorSets) { RAZIX_PROFILE_GPU_SCOPE("Bind Descriptor Sets"); s_APIInstance->BindDescriptorSetsAPImpl(pipeline, cmdBuffer, descriptorSets); }
            RAZIX_FORCE_INLINE static void BindDescriptorSets(RZPipeline* pipeline, RZCommandBuffer* cmdBuffer, RZDescriptorSet** descriptorSets, uint32_t totalSets) { RAZIX_PROFILE_GPU_SCOPE("Bind Descriptor Sets"); s_APIInstance->BindDescriptorSetsAPImpl(pipeline, cmdBuffer, descriptorSets, totalSets); }
            RAZIX_FORCE_INLINE static void BindPushConstants(RZPipeline* pipeline, RZCommandBuffer* cmdBuffer, size_t blockSize, void* data) { RAZIX_PROFILE_GPU_SCOPE("Bind Pushconstants"); s_APIInstance->BindPushConstantsAPIImpl(pipeline, cmdBuffer, blockSize, data); }
            RAZIX_FORCE_INLINE static void Draw(RZCommandBuffer* cmdBuffer, uint32_t count, DataType dataType = DataType::UNSIGNED_INT) { RAZIX_PROFILE_GPU_SCOPE("Draw"); s_APIInstance->DrawAPIImpl(cmdBuffer, count, dataType); }
            RAZIX_FORCE_INLINE static void DrawIndexed(RZCommandBuffer* cmdBuffer, uint32_t indexCount, uint32_t instanceCount = 1, uint32_t firstIndex = 0, int32_t vertexOffset = 0, uint32_t firstInstance = 0) {RAZIX_PROFILE_GPU_SCOPE("Draw Indexed"); s_APIInstance->DrawIndexedAPIImpl(cmdBuffer, indexCount); }

            RAZIX_FORCE_INLINE static void OnResize(uint32_t width, uint32_t height) { RAZIX_PROFILE_GPU_SCOPE("OnResize"); s_APIInstance->OnResizeAPIImpl(width, height); }
