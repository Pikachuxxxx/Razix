...
namespace Razix {
    namespace Graphics {
            ...
            : m_DescriptorPool(VK_NULL_HANDLE)
        {
            ...
            // So we hardcode to create only 2 pools as of now, one for UNIFOR_BUFFER and other for IMAGE_SAMPLER, they can allocate a max os 3 sets so as to facilitate triple buffering
            std::vector<VkDescriptorPoolSize> poolSizes;
            for (size_t i = 0; i < 2; i++) {
                VkDescriptorPoolSize poolSize;
                poolSize.descriptorCount = SWAP_IMAGES_COUNT;
            ...
            poolInfo.maxSets        = static_cast<uint32_t>(SWAP_IMAGES_COUNT);
