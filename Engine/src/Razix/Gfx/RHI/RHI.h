...
#include <stdint.h>
#include <stdio.h>
#include <string.h>    // for strcpy in _rhi_log_timestamp
#include <time.h>
...
#ifdef _WIN32
        // Windows: use thread-safe localtime_s
        static struct tm tm_storage;
        localtime_s(&tm_storage, &now);
        tm_ptr = &tm_storage;
#else
    // Unix/Linux: use standard localtime (universally available)
...
#define rzRHI_DestroyDescriptorTable g_RHI.DestroyDescriptorTable
        ...
        #define rzRHI_BindDescriptorTablesContainer(cb, ppt, rs, dts, N)                             \
            do {                                                                                     \
                RAZIX_PROFILE_SCOPEC("rzRHI_BindDescriptorTables", RZ_PROFILE_COLOR_RHI_DRAW_CALLS); \
                g_RHI.BindDescriptorTables(cb, ppt, rs, dts, N);                                     \
            } while (0)
