...
// Style Guide: This file is a mix of C/C++, it's mostly written in C-flavour but is actually
...
#define RAZIX_SCENE_NODE_NULL         (-1)
#define RAZIX_ZONE_NAME_MAX           64
#define RAZIX_NODE_NAME_MAX           64
#define RAZIX_ZONE_MAX                512
#define RAZIX_SCENE_GRAPH_VERSION     1
...
extern "C"
{
    ...
    // Gather List: Per asset type flat array of active node indices in a zone
    ...
    // Dirty Entry for cache-efficient sorting
    ...
    static_assert(sizeof(rz_scene_node) == RAZIX_CACHE_LINE_SIZE, "rz_scene_node size should be <= RAZIX_CACHE_LINE_SIZE(64) bytes");
    ...
    // Scene Graph Manager, singleton instance is owned by the engine
    typedef struct rz_scene_graph_manager
    {
        rz_scene_graph* pActiveScene;
        rz_scene_graph* pQueuedScenes[8];    // Scene to load next, just pseudo stuff for now, we only have one at a time and App manages this currently
        u32             queuedCount;
    ...
    //-----------------------------------------------------------------------------
    // Node Operations
    // Attach pre-created asset to tree by creating a new node
    RAZIX_API i32 rz_scene_graph_attach_asset(rz_scene_graph* sg, i32 parentNodeIdx, const char* name, rz_uuid uuid, rz_asset_handle handle);
    // Convenience: create asset + attach to tree
    RAZIX_API i32 rz_scene_graph_create_node(rz_scene_graph* sg, i32 parentNodeIdx, const char* name, rz_uuid uuid, Razix::RZAssetType type);
    ...
    RAZIX_API void rz_scene_graph_destroy_node(rz_scene_graph* sg, struct rz_asset_db* pAssetDb, i32 nodeIdx);
    // Node Queries
    ...
    RAZIX_API void rz_scene_graph_traverse_depth_first(rz_scene_graph* sg, i32 startNode, rz_node_visit_fn visit, void* user);
    RAZIX_API void rz_scene_graph_traverse_breadth_first(rz_scene_graph* sg, i32 startNode, rz_node_visit_fn visit, void* user);
    //-----------------------------------------------------------------------------
    // Asset Resolution
    // Called by asset system when async load completes
    RAZIX_API void rz_scene_graph_resolve_asset(rz_scene_graph* sg, i32 nodeIdx, rz_asset_handle resolvedHandle);
    //-----------------------------------------------------------------------------
    // Serialization
    ...
    // TODO: add deserialize functions
    ...
    RAZIX_API bool                     rz_sexp_build_zone(rz_zone* zone, const char* src, const rz_sexp_tokenizer_result* tokens);
    RAZIX_API bool                     rz_sexp_parse_scene_header(rz_scene_graph* sg, const char* src, const rz_sexp_tokenizer_result* tokens);
    ...
    static const u32 s_scene_graph_child_rules[(u32) RZAssetType::COUNT] = {
        ...
        RZ_BIT_SHIFT(RZAssetType::kMesh) |
            ...
            RZ_BIT_SHIFT(RZAssetType::kCamera) |
            RZ_BIT_SHIFT(RZAssetType::kTransform),
        ...
        /* kMaterial */
        RZ_BIT_SHIFT(RZAssetType::kTexture),

        /* kPhysicsMaterial */
        0,

        /* kMesh */
        RZ_BIT_SHIFT(RZAssetType::kMaterial),
        ...
        /* kLuaScript */
        RZ_BIT_SHIFT(RZAssetType::kLuaScript),
