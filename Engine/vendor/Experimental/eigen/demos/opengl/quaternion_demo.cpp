...
public:
    ...
    FancySpheres()
    {
        ...
        static const float angles[10] = {
            0, 0, M_PI, 0. * M_PI, M_PI, 0.5 * M_PI, M_PI, 1. * M_PI, M_PI, 1.5 * M_PI};
        ...
        for (int l = 1; l < levels; l++) {
            ...
            for (int i = start; i < end; ++i) {
                ...
                for (int j = 0; j < 5; ++j) {
                    Vector3f newC = c + ((AngleAxisf(angles[j * 2 + 1], ax0) * AngleAxisf(angles[j * 2 + 0] * (l == 1 ? 0.35 : 0.5), ax1)) * ax0) * (mRadii[i] + radius * 0.8);
...
public:
    EulerAngles() {}
    inline EulerAngles(Scalar a0, Scalar a1, Scalar a2)
        : m_angles(a0, a1, a2) {}
