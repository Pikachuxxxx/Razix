...
//----------------------------------------------------------------------------
/**
 * Calculate the Outgoing radiance for a give light and view position
 * 
 * @param L The Light Direction
 * @param V The View position of the camera/viewer
 * @param N Normal vector of the surface
 * @param F0 Reflectance and normal incidence
 */
float3 DisneyBRDF(float3 L, float3 V, float3 N, float3 F0, float3 albedo, float metallic, float roughness, float3 lightColor, float attenuation)
{
    ...
    Lo = (kD * albedo / Razix::Math::PI + specular) * radiance * NdotL;    // note that we already multiplied the BRDF by the Fresnel (kS) so we won't multiply by kS again
    return Lo;
}
