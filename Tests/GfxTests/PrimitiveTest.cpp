...
{
public:
    PrimitiveTestApp(const RZString& projectRoot, u32 numFrames = TEST_APP_NUM_FRAMES_DEFAULT, const RZString& appName = "RazixGfxTestApp")
        : RZGfxTestAppBase(projectRoot, numFrames, appName) {}

    void OnStart() override
    {
        RZEngine::Get().getWorldRenderer().clearFrameGraph();
        RZEngine::Get().getWorldRenderer().pushRenderPass(&m_PrimitivePass);
...
{
    s_GfxTestBaseApp = new PrimitiveTestApp(RZString(RAZIX_STRINGIZE(RAZIX_ROOT_DIR) + RZString("/Tests/")));
