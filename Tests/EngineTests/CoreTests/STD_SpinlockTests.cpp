...
TEST_F(RZSpinLockTest, Contention_HighLoad)
{
    ...
    auto threadFunc = [&]() {
        for (int i = 0; i < iterations; ++i) {
            testLock.lock();
            // Non-atomic operation protected by lock
            int temp = nonAtomicCounter;
            std::this_thread::yield(); // Increase contention
            nonAtomicCounter = temp + 1;
            testLock.unlock();
        }
        completedThreads++;
    };
    ...
    EXPECT_EQ(nonAtomicCounter, numThreads * iterations);
    EXPECT_EQ(completedThreads.load(), numThreads);
}

TEST_F(RZSpinLockTest, TryLock_UnderContention)
{
    std::atomic<int> tryLockSuccesses{0};
    ...
    // Threads that try to acquire lock
    auto tryerThread = [&]() {
        while (keepRunning) {
    ...
    std::thread holder(holderThread);
    std::thread tryer1(tryerThread);
    std::thread tryer2(tryerThread);
    ...
    EXPECT_GT(tryLockSuccesses.load(), 0);
    EXPECT_GT(tryLockFailures.load(), 0);
}
