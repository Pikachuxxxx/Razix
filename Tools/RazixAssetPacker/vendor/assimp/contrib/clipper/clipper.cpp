...
#define NEAR_EQUAL(a, b) NEAR_ZERO((a) - (b))
    ...
    {
        return (pt1.Y == pt2.Y) ? HORIZONTAL : (double) (pt2.X - pt1.X) / (double) (pt2.Y - pt1.Y);
    }
    //---------------------------------------------------------------------------
    ...
    {
        return (currentY == edge.ytop) ? edge.xtop : edge.xbot + Round(edge.dx * (currentY - edge.ybot));
    }
    //------------------------------------------------------------------------------
    ...
    bool ClipperBase::AddPolygon(const Polygon &pg, PolyType polyType)
    {
        ...
        long64 maxVal;
        if (m_UseFullRange) maxVal = hiRange;
        else
            maxVal = loRange;
    ...
    void Clipper::ProcessHorizontal(TEdge *horzEdge)
    {
        ...
        while (e) {
            ...
            if (eMaxPair ||
                ...
                if (e == eMaxPair) {
                    ...
                    if (dir == dLeftToRight)
                        IntersectEdges(horzEdge, e, IntPoint(e->xcurr, horzEdge->ycurr), (IsTopHorz(e->xcurr)) ? ipLeft : ipBoth);
                    else
                        IntersectEdges(e, horzEdge, IntPoint(e->xcurr, horzEdge->ycurr), (IsTopHorz(e->xcurr)) ? ipRight : ipBoth);
        ...
        if (horzEdge->nextInLML) {
            ...
            if (horzEdge->outIdx >= 0)
                IntersectEdges(horzEdge, eMaxPair, IntPoint(horzEdge->xtop, horzEdge->ycurr), ipBoth);
            assert(eMaxPair);
            if (eMaxPair->outIdx >= 0) throw clipperException("ProcessHorizontal error");
        ...
        while (e) {
            if (IsIntermediate(e, topY)) {
                ...
                if (e->outIdx >= 0 && e->prevInAEL && e->prevInAEL->outIdx >= 0 &&
                    ...
                    SlopesEqual(IntPoint(e->xbot, e->ybot), IntPoint(e->xtop, e->ytop), IntPoint(e->xbot, e->ybot), IntPoint(e->prevInAEL->xtop, e->prevInAEL->ytop), m_UseFullRange)) {
                    ...
                    AddJoin(e, e->prevInAEL);
                           ...
                           SlopesEqual(IntPoint(e->xbot, e->ybot), IntPoint(e->xtop, e->ytop), IntPoint(e->xbot, e->ybot), IntPoint(e->nextInAEL->xtop, e->nextInAEL->ytop), m_UseFullRange)) {
    ...
    void Clipper::BuildResultEx(ExPolygons &polys)
    {
        ...
        while (i < m_PolyOuts.size() && m_PolyOuts[i]->pts) {
            ...
            while (i < m_PolyOuts.size() && m_PolyOuts[i]->pts && m_PolyOuts[i]->isHole) {
                Polygon pg;
                p = m_PolyOuts[i]->pts;
                do {
                    pg.push_back(p->pt);
